...
const ProjectSidebar = ({ onStartAddProject, project, onSelectProject }) => {
  return (
    <aside className="w-full md:w-1/3 px-8 py-16 bg-stone-900 text-stone-50 rounded-r-xl flex flex-col items-center md:items-start">
      ...
      <ul className="w-full flex-grow space-y-4">
        {project.map((project) => {
          let cssClasses =
            "flex items-center p-4  bg-stone-800 rounded-lg shadow-sm hover:shadow-lg transition-shadow duration-300 ease-in-out";
          return (
            <li key={project.id} className={cssClasses}>
              <button
                onClick={() => onSelectProject(project.id)}
                className="flex-grow text-left p-4 text-sm font-medium text-white border-l-4 border-blue-500 hover:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-300 transition-all duration-300 ease-in-out"
              >
                {project.title}
              </button>
            </li>
